...
def create_tables(conn):
	...
	if a.fetchall() == []:
		c.execute('''CREATE TABLE nukes (date text, nukes real)''')
	conn.commit()

def save_nukes_data(conn, update_time, nuke_amount, platform):
	c = conn.cursor()
	c.execute("INSERT INTO nukes VALUES ('{}', {}, '{}')".format(update_time, nuke_amount, platform))
	conn.commit()

def get_nuke_data(conn, platform):
	c = conn.cursor()
	c.execute("select * from nukes where date like '____-__-__ 12:00' and platform='{}'".format(platform))
	results = c.fetchall()
	text = ""
	for i in results:
		text = text + "{" + " x: '{}', y: {}, group:'{}'".format(i[0].split(' ')[0], str(i[1]).replace('.0',''), i[2]) + '},\n'
	return text.rstrip(',')
...
def gather_data():
	...
	conn = sqlite3.connect(os.path.join(dir_path,'nukes.sqlite3'))
	create_tables(conn)
	save_nukes_data(conn, update_time, nuke_amount['stm'], 'stm')
	save_nukes_data(conn, update_time, nuke_amount['ps3'], 'ps3')
	save_nukes_data(conn, update_time, nuke_amount['ps4'], 'ps4')
	nuke_data_steam = get_nuke_data(conn, 'stm')
	nuke_data_ps3 = get_nuke_data(conn, 'ps3')
	nuke_data_ps4 = get_nuke_data(conn, 'ps4')
	conn.close()
